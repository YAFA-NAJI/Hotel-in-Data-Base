...
public class SceneBuilder2c {
    ...
    void handleButtonClick_login(ActionEvent event) throws NumberFormatException, ClassNotFoundException, SQLException {
        ...
        String query = "SELECT * FROM Customer WHERE First_name = ? AND IDC = ?";
        ...
        if (resultSet.next()) {
            FXMLLoader root = new FXMLLoader(getClass().getResource("custumerto.fxml"));
            Pane r;
            try {
                r = root.load();
                Stage primaryStage = new Stage();
            ...
            alert.setContentText("This customer does not exist.");
            alert.showAndWait();
        }
